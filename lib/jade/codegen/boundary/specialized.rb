    ...
    module Boundary
      ...
      module Specialized
        ...
        def specializable_struct(type, registry, seen)
          ...
          new_seen = seen | [struct.qualified_name]
          struct.record_type.fields.values.all? do
            specializable_field?(it, registry, new_seen)
          end ? struct : nil
        end
        ...
        def transitive_closure(seeds, registry)
          seeds.each_with_object(::Set.new(seeds)) do |struct, acc|
            nested_structs(struct, registry).each do |s|
              next if acc.include?(s)

              acc << s
              transitive_closure([s], registry).each { acc << it }
            end
          end.to_a
        end
